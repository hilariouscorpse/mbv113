...
/obj/machinery/power/port_gen/fuel/UseFuel()
	var/datum/reagents/R = Storage.reagents
	for(var/datum/reagent/fuel/F in R.reagent_list)
		F.volume = max((F.volume-1),0)
		playsound(src,'sound/machines/generator_loop.ogg',50,0,8)
	R.update_total()
...
/obj/machinery/power/port_gen/fuel/attack_hand(mob/user)
	if(!active)
		if(HasFuel() && anchored)
			active = 1
			playsound(src,'sound/machines/generator_start.ogg',50,0,8)
			user.visible_message("Generator starts")
	else
		active = 0
		playsound(src,'sound/machines/generator_stop.ogg',50,0,8)
		user.visible_message("Generator stops")