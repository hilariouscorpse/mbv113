...
/datum/preferences
	...
	var/list/custom_names = list("cyborg", "religion", "deity")
	...
	var/unlock_content = 1 //Don't feed the beast! -> unlock_content = C.IsByondMember() That's why everyone has failed to create a functional SS13 remake, after all the countless attempts..
																						//Why bother making it on C++ or C# or damn Unity if you have Byond right?
	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				if("ghostform")
					if(unlock_content)
						var/new_form = input(user, "Choose your ghostly form:","Ghost editor",null) as null|anything in ghost_forms
						if(new_form)
							ghost_form = new_form
				if("ghostorbit")
					if(unlock_content)
						var/new_orbit = input(user, "Choose your ghostly orbit:","Ghost editor", null) as null|anything in ghost_orbits
						if(new_orbit)
